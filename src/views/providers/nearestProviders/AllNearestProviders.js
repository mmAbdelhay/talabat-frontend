    ...
    let curentPosts = this.state.searchedRests.slice(indexOfFirstPost, indexOfLastPost);
       return (
        <div className="container border pt-2 shadow p-3 mb-5 bg-white rounded">
          ...
          <span><Button type="text" onClick={()=>this.handleSortOrder()} className="text-muted">Min.Order Amount</Button></span>
          <span><Button type="text" onClick={()=>this.handleFastest()} className="text-muted">Fastest Delivery</Button></span>
          </div>
          </div>
        <div className="row">
        ...
        {pagesCount>1?<nav aria-label="Page navigation example " className="mt-5">
          <ul class="pagination">
